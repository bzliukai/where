...
function send_request() {
    ...
    if (xmlhttp != null) {
        var serverUrl = config.hostname+'upload/key?dir=leader';
...
//                };
    ...
    FilesAdded: function (up, files) {
        ...
        plupload.each(files, function (file) {
           ...
           // alert(JSON.stringify(file));
            ...
            set_upload_param(uploader, '', false);  //执行上传
        });
    },
    ...
    BeforeUpload: function (up, file) {
        ...
        var _select_pic_list_view = $api.byId('pic_list_view');
        ...
        //<!--单行列表标题效果-->
        // </li>
       $api.append(_select_pic_list_view,str)
    },
...
//                progBar.setAttribute('aria-valuenow', file.percent);
    ...
    FileUploaded: function (up, file, info) {
        if (info.status == 200) {
            //上传完成显示图片
            var img_remote_url = '';
             img_remote_url += file.name+'@-@';
            $api.setStorage('img_remote_url',{
                value:img_remote_url
            });
            // document.getElementById('showImg').setAttribute('src', 'http://outside-leader.oss-cn-qingdao.aliyuncs.com/' + get_uploaded_object_name(file.name));
